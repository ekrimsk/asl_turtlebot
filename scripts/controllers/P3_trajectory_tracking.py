...
class TrajectoryTracker:
    ...
    def compute_control(self, x, y, th, t):
        ...
        xd = V_prev*np.cos(th)      # x vel 
        yd = V_prev*np.sin(th)      # y vel 

        u1 = xdd_d + self.kpx*(x_d - x) + self.kdx*(xd_d - xd)
        u2 = ydd_d + self.kpy*(y_d - y) + self.kdy*(yd_d - yd)

        J_tmp = np.array([[np.cos(th), -V_prev*np.sin(th)],\
                          [np.sin(th),  V_prev*np.cos(th)]])
        tmp_sol = np.linalg.solve(J_tmp, np.array([u1, u2]))
        V = V_prev + dt*tmp_sol[0]  # forward euler 
        om = tmp_sol[1]  # dont need to integrate this, solved directly
        ########## Code ends here ##########
